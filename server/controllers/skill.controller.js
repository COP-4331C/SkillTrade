...
exports.editSkill = async (req, res) => {
  ...
  var changeable_fields = ['title', 'summary', 'description', 'status', 'price', 'country', 'state', 'city'];
...
exports.deleteSkill = async (req, res) => {
  ...
  if (!skill)
    return res.status(400).json({ error: "Invalid skill Id; Skill does not exist."});
  ...
  if (skill.userId != loggedInUser._id)
    return res.status(400).json({ error: "Invalid Credentials: Cannot delete another user's skill."});
  
  // Once we ensure the user is deleting their own skill, then delete it
  Review.deleteOne({_id : skillId})
  .then((data) => {
    res.status(200).json(data);
  })
  .catch((err) => {
