...
exports.createReview = async (req, res) => {
  ...
  if (!author)
    return res
      .status(400)
      .json({ error: "Could not obtain reviewer's information." });

  // Make sure rating is a valid number between 1 and 5
  if (!Number.isInteger(rating) || rating < 1 || rating > 5)
    return res
      .status(400)
      .json({ error: "Invalid Rating; Must be an Integer from 1-5." });
...
exports.getReviews = async (req, res) => {
  ...
  return res.status(200).json(listOfReviews);
};
...
exports.editReview = async (req, res) => {
  ...
  if (!review)
    return res
      .status(400)
      .json({ error: "Invalid review Id; Review does not exist." });
  ...
  if (review.authorId != loggedInUser._id)
    return res
      .status(400)
      .json({
        error: "Invalid Credentials; Cannot edit another user's review.",
      });
...
exports.deleteReview = async (req, res) => {
  ...
  if (review.authorId != loggedInUser._id)
    return res
      .status(400)
      .json({
        error: "Invalid Credentials; Cannot delete another user's review.",
      });
